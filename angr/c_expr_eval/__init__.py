...
def c_expr_eval(c_expr: str, variable_lookup) -> claripy.ast.bv.BV:
    ...
    globals_dict = {"angr": angr, "claripy": claripy, "variable_lookup": variable_lookup}
    return eval(python_expr, globals_dict)  # pylint: disable=eval-used
...
def c_expr_transl(c_expr: str) -> str:
    ...
    Example:
        ...
        returns "claripy.UGT(variable_lookup('global_struct').member('struct_pointer').deref
                 .with_type(angr.types.parse_type('int')).resolved,
                 variable_lookup('pointer2').deref.resolved)"
